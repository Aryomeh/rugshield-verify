...
def normalize_chat_id(cid: int) -> int:
    ...
    s = str(cid)
    if cid > 0:
        # Stored as positive — add -100 prefix
        return int(f"-100{s}")
    if s.startswith("-100"):
        # Already correct
        return cid
    if s.startswith("-"):
        # Has minus but missing 100 — e.g. -1234567 → -1001234567
...
async def verify(body: VerifyRequest):
    ...
    # 3. Resolve chat_id — use the one from the DB token (most reliable)
    #    and normalize it to the correct Telegram supergroup format
    ...
    # 5. Generate invite link and send it directly via bot
    try:
